 ...
 * @brief Micro-benchmark comparing object-model overhead across three representations.
 *
...
/** @brief Volatile sink to prevent benchmark code paths from being optimized away. */
 ...
 * baseline for comparison against Bison binary serialization and JSON text dump.
 */
std::string serialize_native_record(const NativeRecord& obj) {
  std::string buffer;
  buffer.reserve(sizeof(obj.id) + sizeof(obj.age) + sizeof(obj.score) + sizeof(obj.ratio) +
...
/** @brief Deserialize `NativeRecord` from its compact binary blob representation. */
...
std::int32_t json_compute(json& obj, const json& params) {
  ...
  obj["level"] = level;
  obj["active"] = active;
  return obj["id"].get<std::int32_t>() +
         obj["age"].get<std::int32_t>() +
         level +
         params["a"].get<std::int32_t>() +
         params["b"].get<std::int32_t>() +
         static_cast<std::int32_t>(score + obj["ratio"].get<double>()) +
         static_cast<std::int32_t>(obj["name"].get_ref<const std::string&>().size()) +
         (active ? 1 : 0);
}
...
dynamic make_dynamic_method_record(std::size_t iteration) {
  ...
  obj.addMethod("compute"_key, [](dynamic& self, const dynamic& params) -> dynamic {
    ...
    dynamic result;
    result[KEY_VALUE] =
        self[KEY_ID].as<std::int32_t>() +
        self[KEY_AGE].as<std::int32_t>() +
        level +
...
/** @brief One operation row containing statistics for all three representations. */
...
void print_usage(const char* program) {
  std::cout
      << "Usage: " << program << " [iterations] [--iterations=N] [--samples=N]"
  ...
  rows.push_back({
      ...
      measure_stats_stateful(
          ...
          [](dynamic& obj, std::size_t iteration) {
            ...
            obj[KEY_NAME] = std::string{benchmark_name(iteration)};
            consume_i32(obj[KEY_ID].as<std::int32_t>() + obj[KEY_AGE].as<std::int32_t>() + obj[KEY_LEVEL].as<std::int32_t>());
      ...
      measure_stats_stateful(
          ...
          [](json& obj, std::size_t iteration) {
            ...
            obj["level"] = static_cast<std::int32_t>(iteration % 12u);
            obj["name"] = benchmark_name(iteration);
            consume_i32(obj["id"].get<std::int32_t>() + obj["age"].get<std::int32_t>() + obj["level"].get<std::int32_t>());
