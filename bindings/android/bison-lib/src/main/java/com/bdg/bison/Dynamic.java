 ...
 *
 * <h2>Gaps versus the internal C++ API</h2>
 * This initial Android binding covers named-field scalar and vector access,
 * serialization, JSON, and invoking existing methods -- enough for the
 * {@code dynamic obj{"Class"_key, {...}}; obj["field"_key] = value;} quick
 * start and for driving objects over RMI. Not yet exposed, matching gaps the
 * C++ ABI binding documents for its own reasons (see docs/bindings.md):
 * <ul>
 *   <li>Indexed (numeric) field access ({@code obj[0]}) -- named fields only.
 *   <li>Class inheritance ({@code instantiate} with a parent class) and
 *       cross-namespace lookup ({@code findClass}) -- {@link #registerClass}
 *       covers registering a root (parentless) class prototype, enough to
 *       host classes for RMI, but not walking an inheritance chain.
 *   <li>Field/class/method attribute metadata.
 *   <li>YAML text interop (JSON is covered).
 * </ul>
 */
public final class Dynamic implements AutoCloseable {
  ...
  public static void registerClass(Dynamic prototype) {
    nativeAddClass(0, prototype.handle, 0);
  }
  ...
  private static native void nativeAddClass(int nsHash, long protoHandle, int parentHash);
