 ...
 * @brief Linux/MSYS2 half of scoped_terminal_config: termios raw mode plus
 *        the fd-level pass-through pumps (mirrors terminal_posix.cpp's
 *        stdout/stderr redirect, applied to both directions here).
 */
...
/** @brief Writes all of @p data to @p fd, retrying on `EINTR`. @return `false` on error or EOF. */
bool write_all(int fd, std::string_view data) {
  size_t written = 0;
  while (written < data.size()) {
    const ssize_t w = write(fd, data.data() + written, data.size() - written);
    if (w < 0) {
      if (errno == EINTR)
        continue;
      return false;
...
struct scoped_terminal_config::impl {
  // Raw mode, only meaningful if read_fd was a terminal.
...
void scoped_terminal_config::run_output_pump(impl* state) {
  char buf[4096];
  for (;;) {
    const ssize_t n = read(state->output_pipe_read_fd, buf, sizeof(buf));
    if (n <= 0)
...
scoped_terminal_config::impl_ptr scoped_terminal_config::create_state(const params& p) {
  ...
  if (isatty(p.read_fd) != 0 && tcgetattr(p.read_fd, &state->saved_termios) == 0) {
  ...
  // Dup both real fds aside before redirecting either — read_fd and
  // write_fd may be the same fd number (e.g. a pty master/slave), so both
  // dups must happen before the first dup2() below repurposes it.
  state->real_read_fd = dup(p.read_fd);
  state->real_write_fd = dup(p.write_fd);

  // Splice a pipe into read_fd's place; on_passthrough() feeds it.
  int input_pipe_fds[2];
  if (state->real_read_fd >= 0 && pipe(input_pipe_fds) == 0) {
    dup2(input_pipe_fds[0], p.read_fd);
  ...
  if (state->real_write_fd >= 0 && pipe(output_pipe_fds) == 0) {
    state->output_pipe_read_fd = output_pipe_fds[0];
    state->output_pipe_write_fd = output_pipe_fds[1];
    fflush(stdout);
    fflush(stderr);
    dup2(state->output_pipe_write_fd, p.write_fd);
    dup2(state->output_pipe_write_fd, STDERR_FILENO);
...
void scoped_terminal_config::stop_output_pump() {
  ...
  if (state->real_write_fd >= 0) {
    dup2(state->real_write_fd, params_.write_fd);
    dup2(state->real_write_fd, STDERR_FILENO);
  }
  close(state->output_pipe_write_fd); // last writer closed -> pump sees EOF
  ...
  close(state->output_pipe_read_fd);
  if (state->real_write_fd >= 0)
    close(state->real_write_fd);
}
...
void scoped_terminal_config::release_state(impl_ptr state) {
  ...
  if (state->real_read_fd >= 0) {
    if (state->raw_mode_set)
      tcsetattr(state->real_read_fd, TCSANOW, &state->saved_termios);
    dup2(state->real_read_fd, params_.read_fd);
    close(state->real_read_fd);
  }
  if (state->input_pipe_write_fd >= 0 && !state->input_pipe_write_closed)
    close(state->input_pipe_write_fd);
}
...
void scoped_terminal_config::on_passthrough(std::string_view chunk) {
  ...
  if (chunk.empty()) {
    if (!impl_->input_pipe_write_closed) {
      close(impl_->input_pipe_write_fd);
