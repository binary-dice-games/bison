...
/** Dereference a handle to the underlying dynamic object.  Returns nullptr if h is null or the shared_ptr is empty. */
...
BISON_API bison_handle bison_find_class(bison_handle h, bison_hash name) {
  ...
  try {
    ...
    std::shared_lock<std::shared_mutex> lk(bdg::bison::dynamic::getMutex());
    auto& classes = bdg::bison::dynamic::getClasses();
