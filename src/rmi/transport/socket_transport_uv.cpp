...
struct tcp_conn_state {
  ...
  static void on_read(uv_stream_t* stream, ssize_t nread, const uv_buf_t* buf) {
    ...
    while (left > 0) {
      if (st->hdr_pos < 4) {
        ...
        left -= take;
        if (st->hdr_pos == 4) {
          uint32_t net_hdr; std::memcpy(&net_hdr, st->hdr, 4);
      ...
      if (st->hdr_pos == 4 && (left > 0 || st->payload_left == 0)) {
        const size_t take =
            std::min(static_cast<size_t>(st->payload_left), left);
  ...
  static void on_send(uv_async_t* async) {
    ...
    while (!q.empty()) {
      ...
      uv_write(&wr->req,
               reinterpret_cast<uv_stream_t*>(&st->handle),
               &b, 1, on_write_done);
    }
  }
  ...
  void start_loop() {
    loop_thread = std::thread([this] {
      uv_read_start(
          reinterpret_cast<uv_stream_t*>(&handle), alloc_cb, on_read);
  ...
  void enqueue_frame(const bison::buffer& frame) {
    ...
    if (!frame.empty()) std::memcpy(data.data() + 4, frame.data(), frame.size());
    {
...
socket_client_transport::socket_client_transport(std::string host, uint16_t port)
    : impl_(std::make_unique<impl>()) {
  impl_->host = std::move(host);
  impl_->port = port;
}

socket_client_transport::~socket_client_transport() { shutdown(); }
...
void socket_client_transport::open(bison::dynamic params) {
  if (!impl_) throw std::runtime_error("socket_client_transport::open: moved-from");
  if (impl_->opened) return;

  // Allow params to override host/port.
  if (const auto* f = params.findField("host"_key);
      f != nullptr && f->is<std::string>())
    impl_->host = f->as<std::string>();
  if (const auto* f = params.findField("port"_key);
  ...
  struct gai_ctx { struct addrinfo* res{nullptr}; bool done{false}; int status{0}; };
  gai_ctx gai_result;
  gai.data = &gai_result;

  int r = uv_getaddrinfo(&st->loop, &gai,
                         [](uv_getaddrinfo_t* req, int status,
                            struct addrinfo* res) {
  ...
  if (r != 0)
    throw std::runtime_error(
        std::string{"socket_client_transport::open: uv_getaddrinfo: "} +
        uv_strerror(r));

  while (!gai_result.done) uv_run(&st->loop, UV_RUN_NOWAIT);
  ...
  r = uv_tcp_connect(&connect_req, &st->handle, gai_result.res->ai_addr,
  ...
  if (r != 0)
    throw std::runtime_error(
        std::string{"socket_client_transport::open: uv_tcp_connect: "} +
        uv_strerror(r));

  while (!ctx.done) uv_run(&st->loop, UV_RUN_NOWAIT);

  if (ctx.status != 0)
    throw std::runtime_error(
        std::string{"socket_client_transport::open: connect failed: "} +
...
bool socket_client_transport::receive(bison::buffer& frame,
                                      std::chrono::milliseconds timeout) {
  if (!impl_ || !impl_->st) return false;
...
socket_server_connection::~socket_server_connection() { close(); }
...
socket_server_connection::socket_server_connection(std::unique_ptr<impl> i)
    : impl_(std::move(i)) {}
...
bool socket_server_connection::receive(bison::buffer& frame,
                                       std::chrono::milliseconds timeout) {
  if (!impl_ || !impl_->st || impl_->closed) return false;
  return impl_->st->dequeue_frame(frame, timeout);
}
...
struct socket_server_transport::impl {
  ...
  static void on_new_connection(uv_stream_t* server, int status) {
    ...
    if (uv_accept(server,
                  reinterpret_cast<uv_stream_t*>(&cst->handle)) != 0)
      return;
    ...
    auto conn =
        std::make_unique<socket_server_connection>(std::move(conn_impl));
...
socket_server_transport::socket_server_transport(std::string bind_host,
                                                 uint16_t port)
    : impl_(std::make_unique<impl>()) {
...
socket_server_transport::~socket_server_transport() { stop(); }
...
void socket_server_transport::start(bison::dynamic params) {
  if (!impl_) throw std::runtime_error("socket_server_transport::start: moved-from");
  ...
  uv_async_init(&impl_->accept_loop, &impl_->stop_async,
                impl::on_accept_stop);
  impl_->stop_async.data = impl_.get();

  // Bind.
  struct sockaddr_storage addr{};
  int r = uv_ip4_addr(impl_->bind_host.c_str(), impl_->port,
                      reinterpret_cast<struct sockaddr_in*>(&addr));
  if (r != 0)
    r = uv_ip6_addr(impl_->bind_host.c_str(), impl_->port,
                    reinterpret_cast<struct sockaddr_in6*>(&addr));
  if (r != 0)
    throw std::runtime_error(
        std::string{"socket_server_transport::start: address parse failed: "} +
        impl_->bind_host);

  r = uv_tcp_bind(&impl_->acceptor,
                  reinterpret_cast<const struct sockaddr*>(&addr), 0);
  if (r != 0)
    throw std::runtime_error(
        std::string{"socket_server_transport::start: uv_tcp_bind: "} +
        uv_strerror(r));

  r = uv_listen(reinterpret_cast<uv_stream_t*>(&impl_->acceptor), 128,
                impl::on_new_connection);
  if (r != 0)
    throw std::runtime_error(
        std::string{"socket_server_transport::start: uv_listen: "} +
        uv_strerror(r));
...
std::unique_ptr<server_connection_iface> socket_server_transport::accept(
  ...
  if (!impl_->accept_cv.wait_for(lk, timeout, [this] {
        return !impl_->accept_queue.empty() || impl_->stopped.load();
      }))
    return nullptr;
  if (impl_->accept_queue.empty()) return nullptr;
...
void socket_server_transport::stop() {
  if (!impl_ || !impl_->started.load() || impl_->stopped.exchange(true)) return;
