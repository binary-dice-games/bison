...
static std::unique_ptr<named_pipe_server_connection>
make_server_connection(std::unique_ptr<named_pipe_conn> conn);
...
void named_pipe_server_state::on_new_connection(uv_stream_t* server, int status) {
  ...
  if (uv_accept(server, reinterpret_cast<uv_stream_t*>(&cs->handle)) != 0) return;
...
named_pipe_server_connection::~named_pipe_server_connection() { close(); }
...
bool named_pipe_server_connection::receive(bison::buffer& frame,
...
named_pipe_client_transport::~named_pipe_client_transport() { shutdown(); }

void named_pipe_client_transport::open(bison::dynamic /*params*/) {
  if (conn_ && !conn_->closed) return;  // already open

  auto cs = std::make_unique<pipe_conn_state>();
  uv_loop_init(&cs->loop);
  uv_pipe_init(&cs->loop, &cs->handle, 0);

  struct connect_ctx { bool done{false}; int status{0}; };
  connect_ctx ctx;
  cs->handle.data = &ctx;

  uv_connect_t connect_req{};
  connect_req.data = &ctx;
  uv_pipe_connect(&connect_req, &cs->handle, path_.c_str(),
  ...
  while (!ctx.done) uv_run(&cs->loop, UV_RUN_NOWAIT);

  if (ctx.status != 0)
    throw std::runtime_error(
        std::string{"named_pipe_client_transport::open: connect to '"} +
        path_ + "': " + uv_strerror(ctx.status));
...
bool named_pipe_client_transport::receive(bison::buffer& frame,
                                          std::chrono::milliseconds timeout) {
  if (!conn_ || conn_->closed) return false;
  return conn_->cs->dequeue_frame(frame, timeout);
}
...
named_pipe_server_transport::~named_pipe_server_transport() { stop(); }

void named_pipe_server_transport::start(bison::dynamic /*params*/) {
  if (state_) return;  // already started
  ...
  if (r != 0)
    throw std::runtime_error(
        std::string{"named_pipe_server_transport::start: uv_pipe_bind '"} +
        path_ + "': " + uv_strerror(r));

  r = uv_listen(reinterpret_cast<uv_stream_t*>(&ss->listener), 128,
                named_pipe_server_state::on_new_connection);
  if (r != 0)
    throw std::runtime_error(
        std::string{"named_pipe_server_transport::start: uv_listen: "} +
        uv_strerror(r));
...
std::unique_ptr<server_connection_iface> named_pipe_server_transport::accept(
  ...
  if (!state_->accept_cv.wait_for(lk, timeout, [this] {
        return !state_->accept_queue.empty() || stopped_.load();
      }))
    return nullptr;
  if (state_->accept_queue.empty()) return nullptr;
...
void named_pipe_server_transport::stop() {
  if (!state_ || stopped_.exchange(true)) return;
  uv_async_send(&state_->stop_async);
  if (state_->loop_thread.joinable()) state_->loop_thread.join();
  state_.reset();
}
