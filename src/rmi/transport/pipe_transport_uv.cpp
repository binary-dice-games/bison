  ...
  static std::shared_ptr<pipe_channel> create() {
    ...
    if (r != 0) {
      throw std::runtime_error(
          std::string{"pipe_channel: uv_pipe_bind: "} + uv_strerror(r));
    }
    ...
    r = uv_listen(reinterpret_cast<uv_stream_t*>(&listener), 1,
                  [](uv_stream_t* s, int status) {
                    if (status < 0) return;
                    auto* ctx = static_cast<bootstrap_ctx*>(s->data);
                    uv_accept(s,
                        reinterpret_cast<uv_stream_t*>(&ctx->sst->handle));
                    ctx->accepted = true;
                  });
    if (r != 0)
    ...
    uv_pipe_connect(&connect_req, &cst->handle, path.c_str(),
                    [](uv_connect_t* req, int /*status*/) {
                      auto* ctx =
                          static_cast<bootstrap_ctx*>(req->handle->data);
                      ctx->connected = true;
                    });
    ...
    uv_close(reinterpret_cast<uv_handle_t*>(&listener),
             [](uv_handle_t* h) {
               *static_cast<bool*>(h->data) = true;
             });
    while (!listener_closed) uv_run(&sst->loop, UV_RUN_NOWAIT);
...
bool pipe_client_transport::receive(bison::buffer& frame,
...
bool pipe_server_connection::receive(bison::buffer& frame,
                                     std::chrono::milliseconds timeout) {
  if (!ch_) return false;
