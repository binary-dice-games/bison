...
class future {
 public:
  ...
  static future own(rmi_future_handle h) noexcept { return future(h); }

  ~future() { rmi_future_release(f_); }

  future(const future&) = delete;
  future& operator=(const future&) = delete;

  future(future&& o) noexcept : f_(o.f_) { o.f_ = nullptr; }
  ...
  object get_dynamic() {
    ...
    detail::check(
        rmi_future_get_dynamic(&f_, &out), "rmi_future_get_dynamic");
    return object::own(out);
  }

  /** @brief Return the raw handle without transferring ownership. */
  rmi_future_handle get() const noexcept { return f_; }
...
class proxy {
 public:
  ...
  static proxy own(rmi_proxy_handle h) noexcept { return proxy(h); }

  ~proxy() { rmi_proxy_release(p_); }

  proxy(const proxy&) = delete;
  proxy& operator=(const proxy&) = delete;

  proxy(proxy&& o) noexcept : p_(o.p_) { o.p_ = nullptr; }
  ...
  /** @brief Return the raw handle without transferring ownership. */
  rmi_proxy_handle get() const noexcept { return p_; }
   ...
   */
  future call_async(
      client& c, uint32_t method, const object& params = object{});
...
class client {
 public:
  ...
  ~client() { rmi_client_release(h_); }

  client(const client&) = delete;
  client& operator=(const client&) = delete;

  client(client&& o) noexcept : h_(o.h_) { o.h_ = nullptr; }
  ...
  void connect(const object& params = object{}) {
    detail::check(
        rmi_client_connect(h_, params.get()), "rmi_client_connect");
  }
  ...
  object describe(uint32_t klass = 0) const {
    bison_handle out = nullptr;
    detail::check(
        rmi_client_describe(h_, klass, &out), "rmi_client_describe");
    return object::own(out);
  }
  ...
  future describe_async(uint32_t klass = 0) const {
    ...
    detail::check(
        rmi_client_describe_async(h_, klass, &f),
  ...
  future instantiate_async(
      uint32_t klass, const object& params = object{}) {
    rmi_future_handle f = nullptr;
    detail::check(
  ...
  /** @brief Return the raw handle without transferring ownership. */
  rmi_client_handle get() const noexcept { return h_; }
...
inline void proxy::clear(client& c, int64_t timeout_ms) {
  detail::check(
      rmi_proxy_clear(c.get(), p_, timeout_ms), "rmi_proxy_clear");
}
...
inline void proxy::set(client& c, const object& fields, int64_t timeout_ms) {
  detail::check(
      rmi_proxy_set(c.get(), p_, fields.get(), timeout_ms),
      "rmi_proxy_set");
}
...
inline object proxy::get(
    client& c, const object& projection, int64_t timeout_ms) {
  bison_handle out = nullptr;
  detail::check(
...
inline future proxy::call_async(
    client& c, uint32_t method, const object& params) {
  rmi_future_handle f = nullptr;
  detail::check(
...
class server {
 public:
  ...
  ~server() { rmi_server_release(h_); }

  server(const server&) = delete;
  server& operator=(const server&) = delete;

  server(server&& o) noexcept : h_(o.h_) { o.h_ = nullptr; }
  ...
  void listen(const object& params = object{}) {
    detail::check(
        rmi_server_listen(h_, params.get()), "rmi_server_listen");
  }
  ...
  /** @brief Return the raw handle without transferring ownership. */
  rmi_server_handle get() const noexcept { return h_; }
