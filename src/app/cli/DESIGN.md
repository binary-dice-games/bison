...
### Key-name map (inside `on_session`)

`std::unordered_map<uint32_t, std::string>` maps `key_t` hash IDs to their
original string names.  Pre-populated with well-known protocol constants;
extended with every namespace/class/method name the user types.  Used by
`dynamic_to_json()` to emit readable field names in output.
...
- **dynamic → JSON**: local `dynamic_to_json(dynamic, key_name_map)` converts
  ...
  through the key-name map; unresolved hashes render as `"#NNNN"`.
...
- `src/rmi/shared/constants.hpp` — well-known field-name constants.
